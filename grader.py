...
def _clamp_score(score: float) -> float:
    ...
    clamped = float(np.clip(float(score), _SCORE_MIN, _SCORE_MAX))
    # Extra guard against float precision edge cases
    if clamped <= 0.0:
        return _SCORE_MIN
    if clamped >= 1.0:
        return _SCORE_MAX
...
class ModerationGrader:
    ...
    def __init__(self, dataset_path: str = "moderation_dataset.json", seed: int = 42):
    ...
    def grade_all_tasks(self, agent_fn: Callable) -> Dict[str, Any]:
        ...
        raw_agg  = sum(t["score"] for t in task_results.values()) / len(task_results)
        aggregate = _clamp_score(raw_agg)
    ...
    def grade_single_task(self, task_name: str, agent_fn: Callable) -> Dict[str, Any]:
        env = make_task(task_name, self.dataset_path, self.seed)
    ...
    def _run_task(
        self, env: ContentModerationEnv, task_name: str, agent_fn: Callable
    ) -> Dict[str, Any]:
        obs = env.reset()
        ...
        # Always clamp the score from env — double safety net
        ...
        return {
            "task":          task_name,
            "score":         score,           # guaranteed in (0.0001, 0.9999)
    ...
    def _build_summary(self, task_results: Dict[str, Dict], aggregate: float) -> str:
        sep  = "═" * 78
        dash = "─" * 78
        lines = [
            sep,
        ...
        lines += [dash, f" {'AGGREGATE':<10} {aggregate:>7.4f}", sep]
        return "\n".join(lines)
    ...
    def _print_task_detail(self, task_name: str, data: Dict) -> None:
        print(f"\n{'─'*78}\n [{task_name.upper()}] Detailed breakdown\n{'─'*78}")
        cm  = np.array(data["confusion_matrix"])
        clf = data["classification"]
        print(f"\n  Confusion Matrix:")
        for i, action in enumerate(ACTIONS):
            row = " ".join(f"{cm[i,j]:>6d}" for j in range(len(ACTIONS)))
            print(f"  {action:<6} {row}")
        print(f"\n  Per-class: Macro-F1={clf['macro_f1']:.4f}  "
              f"Weighted-F1={clf['weighted_f1']:.4f}")
        print(f"  ECE={data['ece']:.4f}  FNR={data['fnr_high_risk']:.4f}  "
              f"Fairness-Δ={data['fairness_gap']:.4f}")
...
def _classification_report(cm: np.ndarray) -> Dict[str, Any]:
    ...
    weighted_f1 = round(float(sum(f*s for f,s in zip(f1s,sups)) / max(total,1)), 4)
    overall_acc = round(float(cm.diagonal().sum() / max(cm.sum(), 1)), 4)
    return {"per_class": per_class, "macro_f1": macro_f1,
            "weighted_f1": weighted_f1, "overall_acc": overall_acc}


def _expected_calibration_error(pairs: List[Tuple[float, float]], n_bins: int = 10) -> float:
    if not pairs:
        return 0.0
