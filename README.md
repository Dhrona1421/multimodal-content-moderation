...
All results measured at `seed=42`, 12 steps per episode.

### Score (primary metric, 0–1)

| Agent             | Easy   | Medium | Hard   | **Aggregate** |
|-------------------|--------|--------|--------|---------------|
| Random            | ~0.40  | ~0.40  | ~0.40  | ~0.40         |
| PPO (250 updates) | 0.6647 | 0.7498 | 0.7474 | 0.7074        |
| **Rule-Based**    | **0.9289** | **0.8268** | **0.9393** | **0.8983** |
| LLM (GPT-4o-mini) | ~0.92  | ~0.85  | ~0.88  | ~0.88         |
...
> **Why does rule-based beat PPO?** The severity-adjusted reward range `[-1.5, +1.1]` creates an asymmetric incentive: committing to `flag` on every post yields a safe average reward of ~+0.35/step without risking the −1.5 extreme penalty. The PPO policy discovers this conservative strategy during training. The rule-based agent, which was designed with real moderation logic, makes decisive correct calls that score +1.0 per step. This is an honest result — it demonstrates the environment is hard enough that a shallow RL agent with limited training budget cannot trivially surpass a thoughtful deterministic baseline, which is the correct property for a research benchmark.
...
python train.py --eval-only --checkpoint ppo_final
...
3. Add `HF_TOKEN` as a repository secret (optional — enables LLM agent)
4. The Space launches `app.py` automatically on port 7860
...
|-- LICENSE                   # MIT license
|-- ppo_final.npz             # Pre-trained PPO checkpoint (included)
...
- Pre-trained checkpoint `ppo_final.npz` is bundled
- `HF_TOKEN` as a secret enables the LLM chain-of-thought agent
